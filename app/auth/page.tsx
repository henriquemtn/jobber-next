...
export default function AuthPage() {
  ...
  const { mutateAsync } = useMutation(apiLogin) ;

  const enableSocialLogin = process.env.NEXT_PUBLIC_ENABLE_SOCIAL_LOGIN === 'true';
  ...
  const handleSubmit = async (values: IFormValues) => {
    try {
      ...
      // Redirecionar ou exibir um sucesso
      toast.success('Login bem-sucedido!');
      router.push('/home'); // Por exemplo
    } catch (error) {
      console.error(error);
